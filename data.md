...
## 4. Cara Perhitungan Metode Certainty Factor (CF)

Metode Certainty Factor menggunakan rumusan dasar:

$$CF(H, E) = CF(User) \times CF(Pakar)$$

Jika dalam satu penyakit terdapat lebih dari satu gejala yang dipilih user, maka nilai CF digabungkan dengan rumus kombinasi:

$$CF_{combine}(CF_1, CF_2) = CF_1 + CF_2 \times (1 - CF_1)$$

Langkah ini diulang terus untuk semua gejala berikutnya hingga didapatkan nilai akhir $CF_{final}$ untuk penyakit tersebut. Persentase keyakinan dihitung dengan:

$$\text{Persentase} = CF_{final} \times 100\%$$
